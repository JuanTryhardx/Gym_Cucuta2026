...
function drawDonut(canvasId, data, labels, colors) {
    ...
    if (!total) { 
        ...
        ctx.arc(90, 90, 70, 0, Math.PI * 2); ctx.fill(); 
        return; 
    }
    ...
    data.forEach((val, i) => {
        ...
        ctx.arc(90, 90, 70, start, start + slice); ctx.closePath();
    ...
    ctx.fillStyle = '#e2e8f0'; ctx.font = 'bold 22px Rajdhani,sans-serif'; 
...
function drawBar(canvasId, labels, data, color) {
    ...
    for (let i = 0; i <= 4; i++) {
        ...
        ctx.fillStyle = '#64748b'; ctx.font = '10px Nunito,sans-serif'; 
    ...
    labels.forEach((lbl, i) => {
        ...
        if (data[i] > 0) { 
            ctx.fillStyle = '#e2e8f0'; ctx.font = 'bold 11px Nunito,sans-serif'; 
            ctx.textAlign = 'center'; ctx.fillText(data[i], x + barW / 2, y - 5); 
        }
        ctx.fillStyle = '#64748b'; ctx.font = '10px Nunito,sans-serif'; 
        ctx.textAlign = 'center'; ctx.fillText(lbl.slice(0, 6), x + barW / 2, H - pad.bottom + 14);
    });
}
...
window.renderAll = function () {
    ...
    // Gráfico de Planes
    const planesKeys = Object.keys(PLANES); 
    const planesNames = Object.values(PLANES);
    const planData = planesKeys.map(k => todas.filter(p => String(p.plan_id) === k).length);
    const pfN = planesNames.filter((_, i) => planData[i] > 0); 
    const pfD = planData.filter(v => v > 0);
    drawDonut('chartPlan', pfD, pfN, COLORS); 
    buildLegend('legendPlan', pfN, pfD, COLORS);
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        mesData.push(todas.filter(p => { 
            if (!p.fecha_registro) return false; 
            const r = new Date(p.fecha_registro); 
            return r.getFullYear() === y && r.getMonth() === mo; 
        }).length);
    }
    ...
    if(tabla) {
        tabla.innerHTML = planesKeys.map((k, i) => {
            const cnt = planData[i]; 
            const ing = todas.filter(p => String(p.plan_id) === k).reduce((s, p) => s + (parseFloat(p.mensualidad) || 0), 0);
            const pct = ((ing / totalIng) * 100).toFixed(1);
            return `<tr><td>${PLANES[k]}</td><td>${cnt}</td><td>${formatMoney(ing)}</td>
                <td><div style="display:flex;align-items:center;gap:8px">
                <div style="flex:1;height:6px;background:rgba(56,189,248,0.12);border-radius:3px">
                    <div style="width:${pct}%;height:100%;background:#38bdf8;border-radius:3px"></div>
                </div><span style="font-size:0.82rem;color:#64748b">${pct}%</span>
                </div></td></tr>`;
        }).join('');
    }
}
