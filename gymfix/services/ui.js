...
export function buildNavbar(activePage) {
  ...
  if (!user) { window.location.href = '../views/index.html'; return '' }
  ...
  const byRol = {
    cliente:    [{ href: 'eventos.html', label: 'Eventos', icon: '📅' },
                 { href: 'soporte.html', label: 'Soporte', icon: '🛟' }],
    entrenador: [{ href: 'personas.html', label: 'Clientes', icon: '👥' },
                 { href: 'eventos.html',  label: 'Eventos',  icon: '📅' }],
    admin:      [{ href: 'registrar.html',    label: 'Registrar',   icon: '➕' },
