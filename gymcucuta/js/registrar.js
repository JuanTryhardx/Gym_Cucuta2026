...
window.registrarPersona = async function () {
  ...
  const persona = {
    ...
    plan_id:       document.getElementById('r_plan').value,
  ...
  if (error) { console.error(error); showToast('❌ Error: ' + error.message, '#f87171'); return }
...
async function cargarStats() {
  const { data } = await supabase.from('personas').select('estado, mensualidad')
  if (!data) return
...
async function cargarUltimos() {
  const { data } = await supabase.from('personas').select('nombre,estado,fecha_registro').order('id',{ascending:false}).limit(5)
  if (!data) return
